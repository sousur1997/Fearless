...
public class WorkplaceSetup extends AppCompatActivity {
    ...
    private void UpdateWorkspaceToFirebase(){
        ...
        firestore.collection(FearlessConstant.FIRESTORE_WORKPLACE_COLLECTION).document(userId).set(newWorkplace).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if(task.isSuccessful()){
                    //success
                    fearlessLog.sendLog(FearlessConstant.LOG_WORKPLACE_SETUP); //send log to the server
                    saveProgress.setVisibility(View.INVISIBLE);
                    save.setText("Save");
                    save.setEnabled(true);
    ...
    private void getWorkplaceInfo(){
        ...
        save.setEnabled(false);
        if(userId != null) {
            DocumentReference docRef = firestore.collection(FearlessConstant.FIRESTORE_WORKPLACE_COLLECTION).document(userId);
