...
public class ContactListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void addItem(Object item, boolean newItem){
        if(listData.size() >= (FearlessConstant.SOS_NUMBER_COUNT + 2 + FearlessConstant.MAX_CONTACT_TO_ADD)){
            Toast.makeText(context, "Cannot add more than " + FearlessConstant.MAX_CONTACT_TO_ADD + " contacts", Toast.LENGTH_LONG).show();
            return;
        }
        if(!isPresent(item)){
            listData.add(item);
