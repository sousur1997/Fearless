...
public class NearbyAlertService extends Service {
    ...
    public void onCreate() {
        ...
        pnConfiguration.setSubscribeKey(FearlessConstant.SUBSCRIBE_KEY);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if(intent != null){
            ...
            if(intent.getAction().equals(FearlessConstant.STOP_NEARBY_SERVICE)) {
                pubNub.unsubscribe().channels(Arrays.asList(FearlessConstant.CHANNEL_NAME)).execute();
                stopService(intent);
                stopSelf();

            }
            else if(intent.getAction().equals(FearlessConstant.START_NEARBY_SERVICE)) {
//                stopSelf(4);
                //startService(intent);
...
//                        checkIfOutdated();
                ...
                pubNub.subscribe().channels(Arrays.asList(FearlessConstant.CHANNEL_NAME)).execute();
            }
        }
...
//              if it falls under the defined radius, then an alert is issued.
            ...
            NotificationCompat.Builder builder = new NotificationCompat.Builder(this, FearlessConstant.NEARBY_ALERT_CHANNEL)
    ...
    private void sendNearbyAlertBroadcast(String jsonString) {
        ...
        Intent intent = new Intent(FearlessConstant.NEARBY_ALERT_SEND);
