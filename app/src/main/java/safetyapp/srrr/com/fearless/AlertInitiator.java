...
public class AlertInitiator extends Service {
    ...
    public void onCreate() {
        ...
        Intent i = new Intent(FearlessConstant.ALL_SCR_START_BROADCAST_FILTER);
        ...
        initAlertIntent.setAction(FearlessConstant.ALERT_INIT_BROADCAST);
        PendingIntent stopServiceIntent = PendingIntent.getBroadcast(this, 0, initAlertIntent, PendingIntent.FLAG_UPDATE_CURRENT);

        Notification notification = new NotificationCompat.Builder(this, FearlessConstant.INITIATOR_CHANNEL)
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if(intent != null) {
            if (intent.getAction().equals(FearlessConstant.STOP_ALERT)) {
                //alertTask.cancel(true);
                timerStop = true;
                flag_canceled = true;
                alertVibrator.cancel();

                Intent i = new Intent(FearlessConstant.INIT_BROADCAST_FILTER);
                LocalBroadcastManager.getInstance(this).sendBroadcast(i);
            ...
            } else if(intent.getAction().equals(FearlessConstant.START_ALERT)){
                flag_canceled = false;
    ...
    public void startAlert(){
        ...
        alert_init_intent.setAction(FearlessConstant.ACTUAL_START_ALERT);
    ...
    public void startAllScreenService(){
        ...
        acc_Scr_service.setAction(FearlessConstant.START_ALL_SCR);
